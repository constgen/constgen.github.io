...
; (function (browserWindow, nodeGlobal) {
	...
	//NodeJS polyfill
	if (nodeGlobal) {
		location.hostname = require('os').hostname()
		location.search = ''
	}
	...
	Core.isCordova = (!!cordova && /android_asset|^(file:).+www|^local:\/\/\/index.html/.test(location.href)) //detect usage in mobile device WebView with Cordova lib
	...
	Core.Util.relocateCss = function (oldCssLocation, newCssLocation, cssText) {
		...
		return cssText.replace(/url\(\"?'?(.+?)\"?'?\)/g, function (urlExpression, url) {
			...
			if (url.match(/^(http|file|\/)/)) {
				...
				//console.log(11111111111, urlExpression, url)
	...
	Core.URL.rebase = function (baseOld, baseNew) {
	...
	; (function (extendable, global) {
		...
		function setImmediate(callback) {
			return (!!global.setImmediate) ?
				global.setImmediate(callback) //native
				: (global.msSetImmediate && global.msClearImmediate) ? global.msSetImmediate(callback) //IE10
					: global.setTimeout(callback, 1) //emulation
		}

		function clearImmediate(id) {
			return (!!global.clearImmediate) ?
					global.clearImmediate(id) //native
					: global.msClearImmediate ? global.msClearImmediate(callback) //IE10
						: global.clearTimeout(id) //emulation		
		}
		...
		//Determines whether a `value` is the Core.Promise object.
		isPromise = function (value) {
			return value && typeof value === 'object' && typeof value.then === 'function';
		}

		//expose `Promise` constructor and helpers to global scope
		extendable.Promise = Promise
		extendable.isPromise = isPromise
		extendable.every = every
		extendable.any = any
		extendable.some = some
	}(Core, global));//Core is extendable scope
	...
	Core.load = function (src, options) { //returns Promise
		...
		//if (options.reload) {
		//	console.log(Resources.sequence.js)
		//}
		...
		switch (options.type) {
			...
			case 'js':
				LoaderPromise = new Core.Promise(function (loaded, failed, progress) {
					...
					//Sync
						...
						//Add to resource collection
						Resources.add(src, options)
						textContent = Core.request(src, function (err) {
							isCanceled = true
			...
			case 'aac':
				LoaderPromise = new Core.Promise(function (loaded, failed, progress) {
					// Audio loading can't be synchronous, by default it is asynchronous.
				...
				function canceler() {
					if (isLoaded || isCanceled) return
					isCanceled = true
					elem.onload = elem.onabort = elem.onerror = null
					//stop any delayed execution
					clearTimeout(timerId)
					elem.src = '#'
					//to not break sequence
			...
			default:
				LoaderPromise = new Core.Promise(function (loaded, failed, progress) {
					...
					if (options.defer || options.async) {
						...
						elem = Core.requestAsync(src).then(
							function (result) {
					...
					//Sync
						...
						//Add to resource collection
						Resources.add(src, options)
						textContent = Core.template(Core.request(src, function (err) {
							isCanceled = true
	...
	Core.start = function (/*args*/) { //returns Promise object
		...
		while (moduleName = arguments[i++]) {
			...
			if (!module) {
				...
				if (module.initiated && module.body.css && document) {
	...
	Core.stop = function (/*args*/) { //returns Promise object
		...
		while (moduleName = arguments[i++]) {
			...
			//if module has switchable styles
			if (!module.initiated && module.styles && document) {
	...
	Core.config = {
		...
		baseUrl: location.href ?
			(location.protocol + '//' + location.host + location.pathname.substr(0, location.pathname.lastIndexOf('/'))) : '',
		...
		switch (url.substr(format)) {
			...
			case '.js':
				if (window.execScript)
					window.execScript('window.TemporaryCoreVariable = ' + Core.request(url))
				else {
					scriptNode = document.createElement('script')
					scriptNode.setAttribute('type', 'text/javascript')
					scriptNode.innerHTML = 'window.TemporaryCoreVariable = ' + Core.request(url)
	...
	Core.template = function (html, module) {
		...
		*/
		...
		if (module) {
			html = html.replace(/{(module|m):(URL|url)}/g, module.url)
		}

		/*DOM*/
		html = html.replace(/{dom:GET}/g, location.search.substr(1))
		html = html.replace(/{dom:(URL|url)}/g, location.protocol + '//' + location.host + location.pathname)
	...
	Core.XHR = function () {
		...
		if (!HttpRequest) {
			HttpRequest = {}
			HttpRequest.open = HttpRequest.send = function () { }
	...
	Core.request = Core.require = function (url, errback) { //returns text string
		var xhr = new Core.XHR();
		url = Core.template(url)
		try {
	...
	Core.requestAsync = Core.requireAsync = function (url) { //returns Promise
	...
	//DOM manipulations using sandbox.elem()
	Core.DOM = {
		'style': {
			get: function (cssprop) {
				var ret = {},
					//use only first element in collection
					el = this[0];
	...
	//Notifications
	Sandbox.prototype.alert = function () { return Core.Notify.alert.apply(Core.Notify, arguments) }
	Sandbox.prototype.confirm = function () { return Core.Notify.confirm.apply(Core.Notify, arguments) }
	Sandbox.prototype.beep = function () { return Core.Notify.beep.apply(Core.Notify, arguments) }
	Sandbox.prototype.vibrate = function () { return Core.Notify.vibrate.apply(Core.Notify, arguments) }
	Sandbox.prototype.notification = function () { return Core.Notify.alert.notification(Core.Notify, arguments) }
	...
	Sandbox.prototype.data = function (innerData) {
		return {
			format: function (type) {
				if (innerData !== undefined && type in Core.Formats)
					return Core.Formats[type].apply(innerData || false, Array.prototype.slice.call(arguments, 1))
				else
					return innerData
			},
	...
	Sandbox.prototype.listen = function (event, handler) {
		if (event && handler && Modules[this.moduleName]) {
	...
	Sandbox.prototype.elem = (function(){
		var eventAdd = function (event, handler) {
				Sandbox.prototype.Event.add(this, event, handler)
			},
			eventRemove = function (event, handler) {
				Sandbox.prototype.Event.remove(this, event, handler)
			},
			eventFire = function (event, params) {
				Sandbox.prototype.Event.fire(this, event, params)
			},
			//Promise for DOM event. Become resolved when given event type fires first time
			eventPromise = function (event) { //returns Promise
				var isMultiple = false,
					elem = (0 in this && !this.nodeName) ? (isMultiple = true, this) : [this];
				return Core.every(elem.map(function (el) {
					var callback;
					return new Core.Promise(
						function (complete) {
							callback = function () {
								complete(el)
								//remove event listener when Promise resolve
								Sandbox.prototype.Event.remove(el, event, callback)
							}
							//add event listener to resolve Promise
							Sandbox.prototype.Event.add(el, event, callback)
						},
						function oncancel() {
					...
					if (!isMultiple) {// if selector is not multiple return single value
						return result[0]
					}
				})
			},
			...
			// for DOM operations
			domManipulation = function (type) {
				return function () {
					var collection = this, el;
					// `collection` always must be an array
					(0 in collection && !collection.nodeName) || (collection = [collection])

					//special case for `get`
					//determine if first argument is numeric
					if (type == 'get' && !isNaN(parseFloat(arguments[0])) && isFinite(arguments[0])) {
						//then return element with index given in argument
						//but first modify it
						el = collection[arguments[0]]
						if (el) {
							el.on = eventAdd
							el.un = eventRemove
							el.fire = eventFire
							el.when = eventPromise
							el.get = domGet
							el.set = domSet
							el.add = domAdd
							el.remove = domRemove
					...
					//default function
					else if (arguments[0] in Core.DOM && type in Core.DOM[arguments[0]]) {
						//remove first argument and call necessary function
						return Core.DOM[Array.prototype.shift.call(arguments)][type].apply(collection, arguments)
					}
					else
						return {};
				}
			},
			domGet = domManipulation('get'),
			domSet = domManipulation('set'),
			domAdd = domManipulation('add'),
			domRemove = domManipulation('remove'),
			domToggle = domManipulation('toggle'),
			domHas = domManipulation('has'),
			domIs = domManipulation('is');

		return function (selector) { //if no elements in collection returns `null`
			if (!selector) return null
			var nodeCollect;

			//if passed parameter is Node
			if (selector.nodeName) {
				nodeCollect = [selector]
			}
			//if it is Node Collection or Array
			else if (selector instanceof Array || (typeof selector === 'object') && (0 in selector)) {
				nodeCollect = selector
			}
			//else is css-selector string
			else
				nodeCollect = this.root.querySelectorAll(selector);
			
			if (!nodeCollect.length) return null

			//add Array methods
			nodeCollect.forEach = Array.prototype.forEach
			nodeCollect.map = Array.prototype.map
			nodeCollect.filter = Array.prototype.filter
			nodeCollect.every = Array.prototype.every
			nodeCollect.some = Array.prototype.some

			nodeCollect.on = eventAdd
			nodeCollect.un = eventRemove
			nodeCollect.fire = eventFire
			nodeCollect.when = eventPromise

			//nodeCollect.elem = domQuery
		
			nodeCollect.get = domGet
			nodeCollect.set = domSet
			nodeCollect.add = domAdd
			nodeCollect.remove = domRemove
			nodeCollect.has = domHas
			nodeCollect.is = domIs

			return nodeCollect;
		}
	}())
	
	//Expose Core as a module, that compatible witn CommonJS and AMD
	; (function (global, factory) {
		...
		//AMD
			...
			define([], factory)
		//Expose Core global interface object
		} else if (global) {
