/*
Media Query crossbrowser support plugin (polyfill) ver. 0.2.0
...
(function(window,document){
	...
	//function for ajax reques of stylesheet files, returns styles text string
	...
	i=0; while (x = stylesheet[i++])
		if(x.href) stylesText += ajaxRequest(x.href) + ' '
	
	
	//Collect all styles text to one string
	//i=0; while (x = style[i++])
	...
	i=0; while (x = mediaStylesText[i++])  {
		...
		MediaQuery[str] = (MediaQuery[str]) ? MediaQuery[str]+x.substring(x.indexOf('{')+1,x.lastIndexOf('}')) : x.substring(x.indexOf('{')+1,x.lastIndexOf('}'));
	}
