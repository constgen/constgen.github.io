...
Events:	  click, dblclick, mousedown, mouseup, mousemove, mouseover, mouseout,
		  ...
		  mouseenter, mouseleave, mousewheel/wheel, scroll, scrollstart, scrollend
		  push, tap, dbltap, hold, swipe, swipeleft, swiperight, swipetop, swipebottom,
		  pinch,
		  orientationchange, deviceorientation, ready, hashchange
...
/*if(!console){
    var console = {}
    console.log = function(param){return false}
}*/
...
//	} 
	...
	function createPointerUnitEvent(event, type, fix){
		...
		if (!!event.initTouchEvent) {// all browsers with touch screen
			...
			e.fromElement = event.fromElement
					...
					case 'tap':
						...
						if (!elem.uni[type]) {
							...
							if (!TchDevice) {//if not touch device use Click as Tap
								...
								elem.uni[type].start = function(e){
									if (e.target.nodeName == 'A') this.onclick = function(){return false}//cancel click emulation
									addEvent(this, 'touchend', elem.uni[type].handle)
									addClass(elem, 'pushed')
								}
								elem.uni[type].cancel = function(e){
									removeClass(elem, 'pushed')
									this.onclick = null//return click emulation
						...
						if(!elem.uni[type]){
							...
							if (!TchDevice) {//if not touch device use Mousemove
								elem.uni[type].start = function (e) {
									...
									addEvent(elem, 'mousemove', elem.uni[type].handle)
									addClass(elem, 'pushed')
								}
								elem.uni[type].cancel = function(e){
									removeClass(elem, 'pushed')
									removeEvent(elem, 'mousemove', elem.uni[type].handle)
								}
								...
								elem.uni[type].cancel = function(e){removeEvent(elem, 'touchmove', elem.uni[type].handle)}
