	...
	return (function (root, $) {
		...
		var module = {
			...
			init: function () {
				...
				setTimeout(function() {$(".apps.dynamic_grid").addClass("transitionHeight")}, 100);
			...
			animateHeight: function (count) {
				setTimeout(function(){
					...
					if (count == 0) {
						$(".apps.dynamic_grid").css("height", 0);
					}
					else {
						count--;
						var appsHeight = ((count - count%3)/3 + 1)*309 + "px";
						$(".apps.dynamic_grid").css("height", appsHeight);
					}
			...
			heightDontChange: function(count) {
				count--;
				newHeight = ((count - count%3)/3 + 1)*292 + "px";

				if (newHeight == $(".apps.dynamic_grid").css("height")) return true;
				else return false;
			},
