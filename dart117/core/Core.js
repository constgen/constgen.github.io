		...
		switch (options.type) {
			...
			case 'js':
				LoaderPromise = new Core.Promise(function (loaded, failed, progress) {
					...
					//old webkit (before v533.*) has no execution order for dinamicly created scripts
					var brokenOrderBrowser = (function () {
						var match;
						if (!navigator.userAgent) return false;
						match = /(webkit)[\/]([\w.]+)/i.exec(navigator.userAgent)
						//check version of webkit
						if (match && match[2]) return (parseInt(match[2], 10) < 533);
						return false;
					}())
					...
					if (brokenOrderBrowser && options.defer) {
						//NO parallel loading. Scripts will be loaded and executed one by one. But loading does not stop rendering flow.
						var len;
						...
						if (len = Resources.sequence.js.length) {
