...
(function(){
	...
	if(inputTypeRange && (document.getElementById('pricing') || document.getElementById('services')) && input.length) {
		...
		while (el = input[i++]) {
			...
			el.oninput = (function(val){ return function(){
...
(function($){
	...
	function toggleService(){
		...
		if (block.height() <= 70) {
			...
			block.animate({height:(block.children('.shortdescription').eq(0).outerHeight(true)+64)},300)
			block.addClass('active')
		} else {
			block.animate({height:64},300)
	...
	function recountPrice(e){
		...
		if (!isNaN(appPrice))
			...
			if(country == 'USA' || country == 'Canada' || country == 'UK' || country == 'Australia') {
				price = 9.8;
				if (numberReviews >=10) {currentPrice += Math.round(100*(10*price))/100; price = 9;}
				if (numberReviews >=20) {currentPrice += Math.round(100*(10*price))/100; price = 8;}
				if (numberReviews >=30) {currentPrice += Math.round(100*(10*price))/100; price = 7.5;}
				if (numberReviews >=40) {currentPrice += Math.round(100*(10*price))/100; price = 7.5;}
				if (numberReviews >=50) {currentPrice += Math.round(100*(10*price))/100;}
			} else {
				price = 15;
				if (numberReviews >=10) {currentPrice += Math.round(100*(10*price))/100; price = 13;}
				if (numberReviews >=20) {currentPrice += Math.round(100*(10*price))/100; price = 12;}
				if (numberReviews >=30) {currentPrice += Math.round(100*(10*price))/100; price = 10.5;}
				if (numberReviews >=40) {currentPrice += Math.round(100*(10*price))/100; price = 10.5;}
				if (numberReviews >=50) {currentPrice += Math.round(100*(10*price))/100;}
			}
...
(function($){
	...
	function toggleBlock(){
		...
		if (content.height() == 0) {
			content.animate({height:content.children().eq(0).outerHeight(true)},400)
			$(this).parent().addClass('opened')
		} else {
			content.animate({height:0},400)
...
(function($, ValidForm){
	...
	ValidForm.onsubmit = function(form){form.submit()}
	
	function Submit(){
		var form = $('#order .toggleblock.opened form').get(0) || $('#order .toggleblock form').get(0);

		if (!!form.formValidation) form.formValidation()//it trigers or not ValidForm.onsubmit()
		else form.submit()
...
(function($, ValidForm){
	...
	var showValidationMessage = (function($){ return $.Message.show }(ValidForm))
	var hideValidationMessage = (function($){ return $.Message.hide }(ValidForm))
...
;(function($){
	...
	$('#career .switch .button:not(.send)').click(function(){
		...
		$('html, body').animate({scrollTop: article.offset().top+2}, 1000)
		return false
	})
...
(function($){
	...
	var Scrolling = {
		...
		init: function(textscroller, scrollbar, dragger){
			...
			var textscroller_innerHeight = $(textscroller).children('div').eq(0).css({paddingBottom:1}).outerHeight(true) //высота контента в блоке с прокруткой
			...
			$('.arrow-up').mousedown(function(){textscroller.scrollTop+=-55})
			$('.arrow-down').mousedown(function(){textscroller.scrollTop+=55})
			if(touchdevice){
				document.getElementsByClassName('arrow-up')[0].ontouchstart = function(){textscroller.scrollTop+=-55; return false}
				document.getElementsByClassName('arrow-down')[0].ontouchstart = function(){textscroller.scrollTop+=55; return false}
			}
			
			//Prevent page scrolling
			$(textscroller).bind('DOMMouseScroll mousewheel',wheel) 
